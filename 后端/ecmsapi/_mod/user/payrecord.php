...
// 获取当前条件下的总数据量
$total = $api->load('db')->total('[!db.pre!]enewspayrecord' , $map);

// 获取总页数
$totalpage = $total > 0 ? ceil($total/$pagesize) : 1;

// 获取当前页的数据
$list = $total > 0 || $page > $totalpage ? $api->load('db')->select('[!db.pre!]enewspayrecord' , '*' , $map , $pagesize.','.$page , 'posttime desc') : [];

// foreach($list as $i=>$v){
//     // 将时间戳转成 2020-12-11 17:15:10 格式
//     $list[$i]['posttime'] = date('Y-m-d' , $v['posttime']);
// }
