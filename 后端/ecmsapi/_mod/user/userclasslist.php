...
$classname = $api->param('classname',''); // 每页显示数据量
...
if($classname != ''){
	$map .= ' and classname = "'.$classname.'"';
}


// 获取当前条件下的总数据量
$total = $api->load('db')->total('[!db.pre!]enewsuserclass' , $map);

// 获取总页数
$totalpage = $total > 0 ? ceil($total/$pagesize) : 1;

// 获取当前页的数据
$list = $total > 0 || $page > $totalpage ? $api->load('db')->select('[!db.pre!]enewsuserclass' , '*' , $map , $pagesize.','.$page , 'classid desc') : [];
