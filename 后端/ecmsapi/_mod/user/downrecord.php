...
$id = $api->param('id' , 0 , 'intval'); // 信息ID
$classid = $api->param('classid' , 0 , 'intval'); // 栏目ID
...
// 获取当前条件下的总数据量
$total = $api->load('db')->total('[!db.pre!]enewsdownrecord' , $map);

// 获取总页数
$totalpage = $total > 0 ? ceil($total/$pagesize) : 1;

// 获取当前页的数据
$list = $total > 0 || $page > $totalpage ? $api->load('db')->select('[!db.pre!]enewsdownrecord' , '*' , $map , $pagesize.','.$page , 'truetime desc') : [];

foreach($list as $i=>$v){
    // 将时间戳转成 2020-12-11 17:15:10 格式
    $list[$i]['newstime'] = date('Y-m-d' , $v['truetime']);
    $list[$i]['truetime'] = $v['truetime'];
}
