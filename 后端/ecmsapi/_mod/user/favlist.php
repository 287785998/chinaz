...
$cid = $api->param('cid' , 0 , 'intval'); // 指定栏目id
...
if($userid !== 0){
    $map .= ' and [!db.pre!]enewsfava.userid='.$userid;
}


// 获取当前条件下的总数据量
$total = $api->load('db')->total('[!db.pre!]ecms_movie LEFT JOIN [!db.pre!]enewsfava ON [!db.pre!]ecms_movie.id = [!db.pre!]enewsfava.id' , $map);

// 获取总页数
$totalpage = $total > 0 ? ceil($total/$pagesize) : 1;

// 获取当前页的数据
$list = $total > 0 || $page > $totalpage ? $api->load('db')->select('[!db.pre!]ecms_movie LEFT JOIN [!db.pre!]enewsfava ON [!db.pre!]ecms_movie.id = [!db.pre!]enewsfava.id' , '*' , $map , $pagesize.','.$page , '[!db.pre!]enewsfava.favatime desc') : [];

foreach($list as $i=>$v){
    // 将时间戳转成 2020-12-11 17:15:10 格式
    $list[$i]['favatime'] = date('Y-m-d' , $v['favatime']);
