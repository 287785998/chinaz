...
$infotags = $api->param('infotags' , 0 , 'intval'); // 指定栏目id
...
// 关键词搜索
if($infotags != ''){
	$map .= ' and infotags like "%'.$infotags.'%"';
}


// 获取当前条件下的总数据量
$total = $api->load('db')->total('[!db.pre!]ecms_news LEFT JOIN [!db.pre!]ecms_news_data_1 ON [!db.pre!]ecms_news.id=[!db.pre!]ecms_news_data_1.id' , $map);

// 获取总页数
$totalpage = $total > 0 ? ceil($total/$pagesize) : 1;

// 获取当前页的数据
$list = $total > 0 || $page > $totalpage ? $api->load('db')->select('[!db.pre!]ecms_news LEFT JOIN [!db.pre!]ecms_news_data_1 ON [!db.pre!]ecms_news.id=[!db.pre!]ecms_news_data_1.id' , '*' , $map , $pagesize.','.$page , '[!db.pre!]ecms_news.newstime desc') : [];
